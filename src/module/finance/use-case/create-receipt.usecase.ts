 ...
 * 3. Generate receipt code (PT00001...)
...
export class CreateReceiptUseCase {
   ...
   */
  async execute(dto: CreateReceiptDto): Promise<CashTransaction> {
    // 1. Validate input
    ...
    const code = await this.generateCodeUseCase.generateReceiptCode(
      dto.store_id,
    );
    ...
    // 5. Sync cash book (async, không block response)
  ...
  private validateInput(dto: CreateReceiptDto): void {
    ...
    if (!dto.contact_name || dto.contact_name.trim().length === 0) {
      throw new BadRequestException({
        message: 'Tên người nộp tiền không được để trống',
        field: 'contact_name',
      });
    }
    ...
    if (!dto.description || dto.description.trim().length === 0) {
      throw new BadRequestException({
        message: 'Lý do thu tiền không được để trống',
        field: 'description',
      });
    }
